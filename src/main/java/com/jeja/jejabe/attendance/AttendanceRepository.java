...
public interface AttendanceRepository extends JpaRepository<ScheduleAttendance, Long> {
        ...
        // [New] 특정 멤버의 특정 기간 내 'PRESENT' 상태인 출석 횟수 (scheduleDate 기준)
        int countByMemberAndStatusAndScheduleDateBetween(Member member, AttendanceStatus status, LocalDate start,
                        LocalDate end);

        // [New] 특정 멤버의 최근 'PRESENT' 상태인 출석 기록 5개 (scheduleDate 기준)
        List<ScheduleAttendance> findTop5ByMemberAndStatusOrderByScheduleDateDesc(Member member,
        ...
        boolean existsByScheduleAndScheduleDateAndIpAddress(Schedule schedule, LocalDate scheduleDate,
                        String ipAddress);

        // [수정] 특정 날짜의 출석 기록 조회
        Optional<ScheduleAttendance> findByScheduleAndMemberAndScheduleDate(Schedule schedule, Member member,
        ...
        @Query("SELECT sa FROM ScheduleAttendance sa " +
                        ...
                        "AND sa.scheduleDate BETWEEN :startDate AND :endDate " +
                        "AND sa.status = 'PRESENT' " +
                        "ORDER BY sa.scheduleDate DESC")
        List<ScheduleAttendance> findAllByMemberAndDateBetween(
