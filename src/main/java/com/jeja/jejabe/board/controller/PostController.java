...
public class PostController {
    ...
    public ResponseEntity<?> getPosts(@PathVariable String boardKey,
        ...
        return ResponseEntity
                .ok(ApiResponseForm.success(postService.getPostsByBoard(boardKey, keyword, pageable, user),
                        "목록 조회 성공"));
    }

    @GetMapping("/boards/{boardId}")
    @PreAuthorize("@boardGuard.canReadBoard(principal, #boardId)")
    public ResponseEntity<?> getPostsbyBoardId(@PathVariable Long boardId,
            @RequestParam(required = false) String keyword,
            Pageable pageable,
            @AuthenticationPrincipal UserDetailsImpl user) {
        return ResponseEntity
                .ok(ApiResponseForm.success(postService.getPostsByBoard(boardId, keyword, pageable, user), "목록 조회 성공"));
    }

    @PostMapping("/boards/{boardKey}/posts")
    @PreAuthorize("@boardGuard.canWritePost(principal, #boardKey)")
    public ResponseEntity<?> createPost(@PathVariable String boardKey, @RequestBody PostCreateRequestDto dto,
            @AuthenticationPrincipal UserDetailsImpl user) {
        return ResponseEntity.ok(ApiResponseForm.success(postService.createPost(boardKey, dto, user), "등록 완료"));
    }
    ...
    public ResponseEntity<?> getPost(@PathVariable Long postId,
        ...
        return ResponseEntity.ok(ApiResponseForm.success(
                postService.getPostById(postId, user, incrementView),
                "상세 조회 성공"));
    }

    @PatchMapping("/posts/{postId}")
    @PreAuthorize("@boardGuard.canEditPost(principal, #postId)")
    ...
    @PreAuthorize("@boardGuard.canDeletePost(principal, #postId)")
